...
.PHONY: help check check-build check-fmt check-test test generate build clean fmt-check lint audit
...
check-build: generate
	...
	CGO_ENABLED=0 $(GO) build -trimpath -ldflags "$(SWOBU_LDFLAGS)" -o "$$temp_dir/swobu" ./cmd/swobu
