...
func FromString(raw string) Matrix {
...
func FromStringWithViewport(raw string, cols, rows int) Matrix {
	if cols < 1 {
		cols = 1
	}
	if rows < 1 {
	...
	return Matrix{cols: cols, rows: rows, cells: cells}
}

func (m Matrix) Size() (cols, rows int) { return m.cols, m.rows }

func (m Matrix) Cell(x, y int) rune {
...
func (m Matrix) Line(y int) string {
	if y < 0 || y >= m.rows {
		return ""
	}
