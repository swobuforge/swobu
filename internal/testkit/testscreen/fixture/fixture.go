...
const (
	// UpdateEnv enables fixture replacement when set to 1.
...
// When the environment variable named by updateEnv is 1, Compare writes the
// snapshot instead of comparing. Other non-empty values are rejected.
	...
	if err != nil {
		if os.IsNotExist(err) {
			return Report{
				...
				Actual:      snapshot,
				Err: fmt.Errorf(
					"missing visual fixture %q; update with %s=1 go test ./... and review the diff",
					path, updateEnv,
				),
			}
		}
...
func updateEnabled(envKey string) (bool, error) {
	...
	if value != "1" {
		return false, fmt.Errorf("%s must be unset or 1", envKey)
	}
	return true, nil
}
