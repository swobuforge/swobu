...
func TestChatToolResultLossesFollowExchangeCompatibility(t *testing.T) {
	...
	if _, err := EncodeCarrierWithDecisions(request, delivery.BufferedDelivery(), sink, "ex", EncodeOptions{MaxOutputTokensField: MaxOutputTokensFieldCompletion}); err != nil {
		t.Fatal(err)
	}
