...
func TestDecodeProviderDocument_InvalidWireCarrierFailsImmediately(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong media", doc: carrier.NewDocument(protocolkind.ChatCompletions, "text/plain", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "media must be"},
		{name: "missing body", doc: carrier.NewDocument(protocolkind.ChatCompletions, "application/json", nil, nil, carrier.Meta{}), reasonMatch: "body must be configured"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if compatErr.Message != "chat completions response wire carrier is invalid" {
