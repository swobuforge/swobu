...
func TestDecodeProviderEnvelope_InvalidWireCarrierFailsImmediately(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong framing", wire: carrier.CarrierStream{Family: protocolkind.Messages, Frames: carrier.FrameReaderFromReadCloser(io.NopCloser(strings.NewReader(""))), Framing: carrier.FramingNDJSON}, reasonMatch: "framing must be"},
		{name: "missing frames", wire: carrier.CarrierStream{Family: protocolkind.Messages, Framing: carrier.FramingSSE}, reasonMatch: "frames must be configured"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if compatErr.Message != "messages stream wire carrier is invalid" {
