...
func TestMessagesReasoningBudgetKeepsMaxTokensValid(t *testing.T) {
	...
	if _, err := EncodeCarrierWithDecisions(request, delivery.BufferedDelivery(), nil, "", EncodeOptions{}); err == nil {
...
func TestMessagesReasoningRequestRoundTrip(t *testing.T) {
	...
	document, err := EncodeCarrierWithDecisions(decoded.Request.Request, delivery.BufferedDelivery(), nil, "", EncodeOptions{})
	if err != nil {
		t.Fatal(err)
	}
