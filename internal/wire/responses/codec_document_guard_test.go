...
func TestDecodeProviderDocument_InvalidWireCarrierFailsImmediately(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong stage", doc: carrier.NewCarrierDocument(carrier.StageProviderRequestOut, protocolkind.Responses, "application/json", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "stage must be"},
		{name: "wrong media", doc: carrier.NewCarrierDocument(carrier.StageProviderIngressIn, protocolkind.Responses, "text/plain", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "media must be"},
		{name: "missing body", doc: carrier.NewCarrierDocument(carrier.StageProviderIngressIn, protocolkind.Responses, "application/json", nil, nil, carrier.Meta{}), reasonMatch: "body must be configured"},
	}
