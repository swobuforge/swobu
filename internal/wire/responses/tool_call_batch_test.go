...
func TestDecodeRequest_DecodesParallelToolCalls(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "omitted",
			raw:  `{"model":"gpt-4o-mini","input":"hi"}`,
			want: canonical.ToolCallBatchUnspecified,
		},
		{
			name: "true means unspecified",
			raw:  `{"model":"gpt-4o-mini","input":"hi","parallel_tool_calls":true}`,
			want: canonical.ToolCallBatchUnspecified,
		},
		{
			name: "false lowers to at_most_one",
			raw:  `{"model":"gpt-4o-mini","input":"hi","parallel_tool_calls":false}`,
			want: canonical.ToolCallBatchAtMostOne,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, _, err := codec.DecodeClientRequest(carrier.CarrierDocument{Family: protocolkind.Responses, Raw: []byte(tc.raw)})
			if err != nil {
...
func TestDecodeRequest_RejectsParallelToolCallsWrongType(t *testing.T) {
	codec := legacyClientRequestDecoder{}
	_, _, err := codec.DecodeClientRequest(carrier.CarrierDocument{Family: protocolkind.Responses, Raw: []byte(`{"model":"gpt-4o-mini","input":"hi","parallel_tool_calls":"nope"}`)})
	if err == nil {
