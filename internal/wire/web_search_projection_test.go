...
func TestResponsesUndisclosedSourcesOmitClaudeSearchAccountingWithoutBaseUsage(t *testing.T) {
	...
	if bytes.Contains(output, []byte(`"usage"`)) {
		t.Fatalf("Claude search accounting fabricated unknown base usage: %s", output)
	}
}
