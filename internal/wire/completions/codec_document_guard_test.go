...
func TestDecodeProviderDocument_InvalidWireCarrierFailsImmediately(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong stage", doc: carrier.NewCarrierDocument(carrier.StageProviderRequestOut, protocolkind.Completions, "application/json", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "stage must be"},
		{name: "wrong media", doc: carrier.NewCarrierDocument(carrier.StageProviderIngressIn, protocolkind.Completions, "text/plain", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "media must be"},
		{name: "missing body", doc: carrier.NewCarrierDocument(carrier.StageProviderIngressIn, protocolkind.Completions, "application/json", nil, nil, carrier.Meta{}), reasonMatch: "body must be configured"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if compatErr.Message != "completions response wire carrier is invalid" {
