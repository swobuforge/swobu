...
func TestEncode_RejectsInstructions(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "instructions") {
		t.Fatalf("EncodeCarrier err=%v, want instructions rejection", err)
	}
}

func TestDecodeRequest_RejectsInstructions(t *testing.T) {
	codec := legacyClientRequestDecoder{}
	req := []byte(`{"model":"gpt-3.5","instructions":"Use native tools.","prompt":"hi"}`)
	_, _, err := codec.DecodeClientRequest(carrier.CarrierDocument{Family: protocolkind.Completions, Raw: req})
	if err == nil || !strings.Contains(err.Error(), "instructions") {
		t.Fatalf("DecodeClientRequest err=%v, want instructions rejection", err)
	}
}
