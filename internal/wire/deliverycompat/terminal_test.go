...
func (s *recordingSink) Commit(_ context.Context, _ string, effects []compat.Decision) error {
	s.effects = append([]compat.Decision(nil), effects...)
	return nil
}
...
func TestEmitTerminalUsagePresence(t *testing.T) {
	...
	exactEffect := exactSink.effects[0]
