...
// domain validation and operator-facing setup surfaces, plus capability facts
// consumed by request-path policy orchestration.
//
...
//
// Cache control capability facts and tool-choice capability facts are also
// owned here and are route-scoped (`provider + protocol` or `provider +
// protocol + model`), not provider-marketing scoped.
//
