...
func TestRoot_ScrollVisibilityProofMatrix(t *testing.T) {
	...
	type proofCase struct {
		name     string
		viewport geom.Rect
		run      func(t *testing.T, rt *loop.AppLoop[state.Model], viewport geom.Rect)
		assert   []string
	}
...
func dispatchKeys(rt *loop.AppLoop[state.Model], viewport geom.Rect, keys ...interaction.Key) {
