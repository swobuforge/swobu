...
func trafficOpenDetailRows(row state.TrafficRow) []retained.ViewSpec[state.Model] {
	rows := []retained.ViewSpec[state.Model]{
		trafficDetailLine("request id", strings.TrimSpace(row.RequestID)), // swobu:io-string source=boundary
		...
		trafficDetailLine("route", strings.TrimSpace(row.Target)), // swobu:io-string source=boundary
