...
func TestProviderMatrixProjectsSemanticSetupStatus(t *testing.T) {
	tests := []struct {
		...
		{"custom missing backend", profile.ProviderSpecOpenAICompatible, "", "", setupMissingLocator},
		{"custom remote missing credential", profile.ProviderSpecOpenAICompatible, "https://example.com/v1", "", setupMissingCredential},
		{"custom loopback ready", profile.ProviderSpecOpenAICompatible, "http://127.0.0.1:8080/v1", "", setupReady},
