...
func (f *azureProviderForm) Render(app *tui.App) *tui.Element {
	__tui_0 := tui.New(
	...
	if shouldRenderEndpointRow(f.target) {
		__tui_1 := app.Mount(f, 0, func() tui.Component {
			return AzureProjectEndpointDraftInput(f.target, f.endpointDraft, setupRequiresLocator(f.target), func() { f.endpointCommitted = true })
		})
		__tui_0.AddChild(__tui_1)
	}
	if setupRequiresLocator(f.target) {
		__tui_2 := app.Mount(f, 1, func() tui.Component {
			return AzureCredentialBlocked(f.target)
		})
		...
		__tui_4 := app.Mount(f, tui.MountKey(2, credentialRegionKey(f.target)), func() tui.Component {
			return CredentialControlRegion(f.target, f.endpointCommitted)
		})
