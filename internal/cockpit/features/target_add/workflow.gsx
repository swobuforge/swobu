...
templ (w *Workflow) Render() {
	<div class="flex-col w-full" deps={w.Phase, w.Provider, w.CredentialRef, w.ProviderSetup, w.AuthSession, w.CatalogLoading, w.SelectedModel, w.ManualModelID, w.Error, len(w.Route.Targets)}>
		if w.IsOpen() {
			...
			<div class="flex-col w-full">
				...
				if w.Phase.Get() == PhaseReadyToCreate {
					...
					if w.routeHasTargets() {
						@PlacementDisplayRowComponent(w)
					} else {
						@FixedPlacementRowComponent(w)
					}
