...
func TestDecodeResponseBuffered_MapsUsageAndCacheFields(t *testing.T) {
	raw := []byte(`{
		...
		"usage":{"prompt_tokens":44,"completion_tokens":3,"completion_tokens_details":{"reasoning_tokens":1},"prompt_tokens_details":{"cached_tokens":20,"cache_write_tokens":2}}
	}`)
	...
	reasoningEffect, ok := sink.effects[2].(effect.Compatibility)
	if !ok {
	...
	cacheReadEffect, ok := sink.effects[3].(effect.Compatibility)
	if !ok {
	...
	cacheWriteEffect, ok := sink.effects[4].(effect.Compatibility)
	if !ok {
		t.Fatalf("captured effect[4] type = %T, want effect.Compatibility", sink.effects[4])
	}
...
func TestDecodeResponseStream_EmitsUsageBeforeTerminalDecision(t *testing.T) {
	raw := "data: {\"id\":\"cmpl_1\",\"model\":\"m\",\"choices\":[{\"text\":\"ok\",\"finish_reason\":\"\"}],\"usage\":{\"completion_tokens\":3}}\n\n" +
