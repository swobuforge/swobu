...
func TestDecodeProviderDocument_InvalidWireCarrierFailsImmediately(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong stage", doc: carrier.NewWireDocument(carrier.StageProviderRequestOut, protocolkind.Completions, "application/json", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "stage must be"},
		{name: "wrong media", doc: carrier.NewWireDocument(carrier.StageProviderIngressIn, protocolkind.Completions, "text/plain", nil, []byte(`{"ok":true}`), carrier.Meta{}), reasonMatch: "media must be"},
		{name: "missing body", doc: carrier.NewWireDocument(carrier.StageProviderIngressIn, protocolkind.Completions, "application/json", nil, nil, carrier.Meta{}), reasonMatch: "body must be configured"},
	}
