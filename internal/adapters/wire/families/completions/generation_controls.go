...
func decodeCompletionsGenerationControls(dto completionsRequestDTO) (canonical.GenerationControls, error) {
	maxTokens, err := decodeOptionalIntMessage(dto.MaxTokens, "completions request max_tokens is invalid")
	if err != nil {
		return canonical.GenerationControls{}, err
	}
	temperature, err := decodeOptionalFloatMessage(dto.Temperature, "completions request temperature is invalid")
	if err != nil {
		return canonical.GenerationControls{}, err
	}
	topP, err := decodeOptionalFloatMessage(dto.TopP, "completions request top_p is invalid")
	if err != nil {
		return canonical.GenerationControls{}, err
	}
	stopSequences, err := decodeStopSequences(dto.Stop, "completions request stop is invalid")
	if err != nil {
		return canonical.GenerationControls{}, err
	}
