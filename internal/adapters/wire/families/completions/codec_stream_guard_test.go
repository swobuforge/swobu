...
func TestDecodeProviderStream_InvalidWireCarrierFailsImmediately(t *testing.T) {
	tests := []struct {
		...
		{name: "wrong protocol", wire: carrier.WireStream{Family: protocolkind.ChatCompletions, Body: io.NopCloser(strings.NewReader("")), Framing: carrier.FramingSSE}, reasonMatch: "protocol must be"},
		{name: "wrong framing", wire: carrier.WireStream{Family: protocolkind.Completions, Body: io.NopCloser(strings.NewReader("")), Framing: carrier.FramingNDJSON}, reasonMatch: "framing must be"},
		{name: "missing frames", wire: carrier.WireStream{Family: protocolkind.Completions, Framing: carrier.FramingSSE}, reasonMatch: "body must be configured"},
	}
