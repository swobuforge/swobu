...
func (MessagesFamilyCodec) DecodeRequest(raw []byte) (canonical.CanonicalRequest, bool, error) {
	...
	if err := json.Unmarshal(raw, &dto); err != nil {
		return canonical.CanonicalRequest{}, false, canonical.BadRequest("messages request body is invalid JSON")
	}
	if len(dto.Messages) == 0 {
		return canonical.CanonicalRequest{}, false, canonical.BadRequest("messages request is missing required fields")
	}
...
func (MessagesFamilyCodec) EncodeResponse(output canonical.CanonicalOutput) ([]byte, error) {
	items := output.Items()
	...
	stopReason := "end_turn"
	if streamwire.ContainsToolUseOutput(items) {
		stopReason = "tool_use"
	}
	return json.Marshal(messagesResponseDTO{
		ID:         streamwire.FallbackID(output.ResultID(), "msg_swobu"),
		Type:       "message",
		Role:       "assistant",
		Model:      output.Model(),
		Content:    content,
...
func (MessagesFamilyCodec) EncodeRequest(request canonical.CanonicalRequest, streaming bool) (core.WireRequest, error) {
	return encodeRequest(request, streaming)
}

func (MessagesFamilyCodec) DecodeResponse(raw []byte) (canonical.CanonicalOutputValue, error) {
	return decodeResponseBuffered(raw)
}

func (MessagesFamilyCodec) DecodeResponseStream(body io.ReadCloser, exchangeID string) canonical.EventReader {
	return decodeResponseStream(body, exchangeID)
}

func (MessagesFamilyCodec) NewStreamState() streamwire.EnvelopeStreamEncoder {
	return &messagesEnvelopeStreamEncoder{adapter: streamwire.NewEnvelopeEventAdapter()}
}
...
func decodeMessagesItems(raw json.RawMessage, msgIdx int, role string) ([]canonical.CanonicalItem, error) {
	...
	for _, part := range parts {
		...
		case "tool_use":
			...
			input, err := streamwire.DecodeJSONObject(part.Input, "messages request tool_use input is invalid")
			if err != nil {
				return nil, err
			}
...
func (s *messagesEnvelopeStreamEncoder) Encode(event streamwire.StreamEvent) ([][]byte, error) {
	if s.blockIndexByID == nil {
		s.blockIndexByID = map[string]int{}
	}
	switch event.Kind {
	case streamwire.StreamEventStarted:
		s.started = true
		raw, _ := json.Marshal(messagesStartEventDTO{
			Type: "message_start",
			Message: messagesStartMessageDTO{
				ID:   streamwire.FallbackID(event.ResultID, "msg_swobu"),
				Role: "assistant",
			},
		})
		return [][]byte{
			streamwire.SSEEventFrame("message_start", raw),
		}, nil
	case streamwire.StreamEventItemStarted:
		if !s.started {
			frames, _ := s.Encode(streamwire.StreamEvent{Kind: streamwire.StreamEventStarted, ResultID: event.ResultID, Model: event.Model})
		...
		case canonical.ItemKindText:
			...
			return [][]byte{streamwire.SSEEventFrame("content_block_start", raw)}, nil
		case canonical.ItemKindToolUse:
			s.sawToolUse = true
			raw, _ := json.Marshal(messagesContentBlockStartDTO{
				Type:  "content_block_start",
	...
	case streamwire.StreamEventTextDelta:
		if !s.started || s.activeTextID == "" {
			frames, _ := s.Encode(streamwire.StreamEvent{
				Kind:     streamwire.StreamEventItemStarted,
				ItemKind: canonical.ItemKindText,
				ResultID: event.ResultID,
				Model:    event.Model,
				ItemID:   streamwire.FallbackID(event.ItemID, "text_0"),
			})
			more, err := s.Encode(event)
			return append(frames, more...), err
		}
		...
		return [][]byte{streamwire.SSEEventFrame("content_block_delta", raw)}, nil
	case streamwire.StreamEventToolUseArgumentsDelta:
		...
		return [][]byte{streamwire.SSEEventFrame("content_block_delta", raw)}, nil
	case streamwire.StreamEventItemCompleted:
		index, ok := s.blockIndexByID[event.ItemID]
		if !ok {
			return nil, nil
		}
		...
		return [][]byte{streamwire.SSEEventFrame("content_block_stop", raw)}, nil
	case streamwire.StreamEventCompleted:
		...
		for _, index := range s.blockIndexByID {
			raw, _ := json.Marshal(messagesContentBlockStopDTO{Type: "content_block_stop", Index: index})
			frames = append(frames, streamwire.SSEEventFrame("content_block_stop", raw))
		}
		if s.sawToolUse {
			...
			frames = append(frames, streamwire.SSEEventFrame("message_delta", raw))
		}
		raw, _ := json.Marshal(struct {
			Type string `json:"type"`
		}{Type: "message_stop"})
		frames = append(frames, streamwire.SSEEventFrame("message_stop", raw))
		s.blockIndexByID = map[string]int{}
		s.sawToolUse = false
		return frames, nil
	default:
