...
func (ChatCompletionsFamilyCodec) DecodeRequest(raw []byte) (canonical.CanonicalRequest, bool, error) {
	...
	if err := json.Unmarshal(raw, &dto); err != nil {
		return canonical.CanonicalRequest{}, false, canonical.BadRequest("chat completions request body is invalid JSON")
	}
	if len(dto.Messages) == 0 {
		return canonical.CanonicalRequest{}, false, canonical.BadRequest("chat completions request is missing required fields")
	}
...
func (ChatCompletionsFamilyCodec) EncodeResponse(output canonical.CanonicalOutput) ([]byte, error) {
	return json.Marshal(chatCompletionsResponseDTO{
		ID:     streamwire.FallbackID(output.ResultID(), "chatcmpl_swobu"),
...
func (ChatCompletionsFamilyCodec) EncodeRequest(request canonical.CanonicalRequest, streaming bool) (core.WireRequest, error) {
	return encodeRequest(request, streaming)
}

func (ChatCompletionsFamilyCodec) DecodeResponse(raw []byte) (canonical.CanonicalOutputValue, error) {
	return decodeResponseBuffered(raw)
}

func (ChatCompletionsFamilyCodec) DecodeResponseStream(body io.ReadCloser, exchangeID string) canonical.EventReader {
	return decodeResponseStream(body, exchangeID)
}

func (ChatCompletionsFamilyCodec) NewStreamState() streamwire.EnvelopeStreamEncoder {
	return &chatCompletionsEnvelopeStreamEncoder{adapter: streamwire.NewEnvelopeEventAdapter()}
}
...
func decodeChatCompletionsItems(
	...
	for idx, call := range toolCalls {
		...
		input, err := streamwire.DecodeJSONObject(call.Function.Arguments, "chat completions tool call arguments are invalid")
		if err != nil {
			return nil, err
		}
...
func (s *chatCompletionsEnvelopeStreamEncoder) Encode(event streamwire.StreamEvent) ([][]byte, error) {
	if s.toolByID == nil {
		s.toolByID = map[string]int{}
	}
	switch event.Kind {
	case streamwire.StreamEventStarted:
		s.resultID = streamwire.FallbackID(event.ResultID, "chatcmpl_swobu")
		s.model = event.Model
		s.started = true
	...
	case streamwire.StreamEventItemStarted:
		if event.ItemKind == canonical.ItemKindToolUse {
			...
			raw, _ := json.Marshal(chatCompletionsResponseDTO{
				ID:     streamwire.FallbackID(s.resultID, "chatcmpl_swobu"),
				Object: "chat.completion.chunk",
				Model:  s.model,
				Choices: []chatCompletionsChoiceDTO{{
					Index: 0,
	...
	case streamwire.StreamEventTextDelta:
		if !s.started {
			frames, _ := s.Encode(streamwire.StreamEvent{Kind: streamwire.StreamEventStarted, ResultID: s.resultID, Model: s.model})
	...
	case streamwire.StreamEventToolUseArgumentsDelta:
		...
		if !ok {
			startFrames, err := s.Encode(streamwire.StreamEvent{
