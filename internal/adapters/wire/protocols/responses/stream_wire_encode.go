...
func (e *ResponseStreamWireEncoder) Encode(event streamwire.StreamEvent) ([][]byte, error) {
	switch event.Kind {
	case streamwire.StreamEventStarted:
		e.responseID = streamwire.FallbackID(event.ResultID, "resp_swobu")
		e.model = event.Model
	...
	case streamwire.StreamEventCompleted:
		...
		if err != nil {
			...
			Response: responsesStreamingResponseDTO{
				ID:     streamwire.FallbackID(e.responseID, "resp_swobu"),
				Object: "response",
				Model:  e.model,
				Status: "completed",
...
func (e *ResponseStreamWireEncoder) encodeItemStarted(event streamwire.StreamEvent) ([][]byte, error) {
...
func (e *ResponseStreamWireEncoder) encodeTextDelta(event streamwire.StreamEvent) ([][]byte, error) {
	itemID := strings.TrimSpace(event.ItemID) // swobu:io-string source=boundary
...
func (e *ResponseStreamWireEncoder) encodeToolArgumentsDelta(event streamwire.StreamEvent) ([][]byte, error) {
	itemID := strings.TrimSpace(event.ItemID) // swobu:io-string source=boundary
	if itemID == "" {
...
func (e *ResponseStreamWireEncoder) encodeItemCompleted(event streamwire.StreamEvent) ([][]byte, error) {
	switch event.ItemKind {
	case canonical.ItemKindText:
		itemID := strings.TrimSpace(event.ItemID) // swobu:io-string source=boundary
