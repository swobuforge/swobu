...
// NewRuntime retains the shared Bearer transport and OpenAI-shaped model
// catalog while replacing only Chat's provider-owned reasoning carrier.
func NewRuntime(client *http.Client, credentials providersruntime.CredentialProvider) providersruntime.ProviderRuntimeBundle {
	bundle := openaifamily.NewRuntime(client, credentials, openaifamily.StandardBearerPolicy(profile.ProviderSpecCerebras))
...
func (r backendResolver) ResolveBackend(target provider.TargetSnapshot) (provider.Backend, error) {
	...
	if target.ProtocolKind != protocolkind.ChatCompletions {
		return provider.Backend{}, fmt.Errorf("Cerebras backend protocol %q is not Chat Completions", target.ProtocolKind)
	}
	standard, ok := backend.Codec.(protocolcodec.Codec)
	if !ok {
		return provider.Backend{}, fmt.Errorf("Cerebras Chat backend has codec %T, want protocolcodec.Codec", backend.Codec)
	}
