...
func TestEncode_OmitsInputForContinuationOnlyRequests(t *testing.T) {
	req := compatibility.NewGenerationRequest(compatibility.GenerationRequestParams{
		...
		Thread:             []compatibility.CanonicalItem{compatibility.NewTextItem(compatibility.ItemAuthorAssistant, "prior output")},
		LastTurn:           nil,
		PreviousResponseID: "resp_123",
	})

	wire, err := Encode(req, compatibility.DeliveryModeBuffered)
	if err != nil {
...
func TestEncode_KeepsLastTurnInputWithPreviousResponseID(t *testing.T) {
	req := compatibility.NewGenerationRequest(compatibility.GenerationRequestParams{
		...
		Thread: []compatibility.CanonicalItem{
			compatibility.NewTextItem(compatibility.ItemAuthorAssistant, "prior output"),
			compatibility.NewTextItem(compatibility.ItemAuthorUser, "new user turn"),
		},
