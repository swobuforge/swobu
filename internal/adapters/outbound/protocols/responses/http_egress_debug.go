...
func logResponsesEgressStreamFrame(raw []byte) {
	...
	slog.Debug("responses stream egress frame",
		"component", "httpapi",
		"event", "responses_stream_egress_frame",
		"frame_type", strings.TrimSpace(frameType), // trimlowerlint:allow boundary canonicalization
...
func compactAndTruncateJSON(raw []byte, maxBytes int) (string, bool) {
	text := strings.TrimSpace(string(raw)) // trimlowerlint:allow boundary canonicalization
	if text == "" {
		return "null", false
	}
	normalized := text
