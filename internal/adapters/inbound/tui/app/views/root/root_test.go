...
func TestRoot_OnMountStartsDaemonRefreshLoop(t *testing.T) {
	...
	if len(effects) != 1 {
		t.Fatalf("on-mount effect count = %d, want 1", len(effects))
	}
	if _, ok := effects[0].(state.ScheduleDaemonRefreshEffect); !ok {
		t.Fatalf("on-mount effect = %T, want state.ScheduleDaemonRefreshEffect", effects[0])
	}
}
...
func newTestRuntime(model state.Model) *runtime.AppLoop[state.Model] {
	return runtime.New(model, state.Reduce)
}
...
func focusRowContaining(t *testing.T, rt *runtime.AppLoop[state.Model], viewport geom.Rect, pattern string) {
	t.Helper()
	for i := 0; i < 20; i++ {
		out := rt.Render(viewport).String()
		for _, line := range strings.Split(out, "\n") {
...
func selectClientFromChooser(t *testing.T, rt *runtime.AppLoop[state.Model], viewport geom.Rect, label string) {
	t.Helper()
	focusRowContaining(t, rt, viewport, "clients")
	rt.DispatchEvent(updateKey(interaction.KeyEnter))
	rt.Rebuild(Root(), viewport)
