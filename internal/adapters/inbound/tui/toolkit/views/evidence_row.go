...
func RenderEvidenceRow(width int, spec EvidenceRowSpec) string {
	...
	base := fmt.Sprintf(
		"%s   %-8s %-11s %-19s %7s   %-8s",
		marker,
		trimToWidth(strings.TrimSpace(spec.Time), 8),
		trimToWidth(strings.TrimSpace(spec.Kind), 11),
		trimToWidth(strings.TrimSpace(spec.Route), 19),
		trimToWidth(strings.TrimSpace(spec.Timing), 7),
		trimToWidth(strings.TrimSpace(spec.Result), 8),
	)
	if action := strings.TrimSpace(spec.Action); action != "" {
		base += " " + action
	}
