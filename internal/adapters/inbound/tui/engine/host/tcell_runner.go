...
func New[M any](screen tcell.Screen, root view.ViewSpec[M], model M, reduce runtime.Reducer[M]) *Runner[M] {
	runner := &Runner[M]{
		Screen:             screen,
		Root:               root,
		foregroundRequests: make(chan foregroundHandoffCall),
	}
	runner.Loop = runtime.New(model, reduce)
	return runner
}
