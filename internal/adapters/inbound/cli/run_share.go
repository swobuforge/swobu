...
func runShare(ctx context.Context, client *http.Client, stdout, stderr io.Writer, args []string) ExitCode {
	...
	ref := ""
	expiry := sharestate.ExpiryOneDay
	...
	result, err := operatorclient.New(client, platformconfig.BaseURL(startup.Addr)).IssueShare(ctx, ref, expiry)
	if err != nil {
		fmt.Fprintln(stderr, err)
		return ExitDown
	}
	if expiry != sharestate.ExpiryOneDay {
		fmt.Fprintln(stdout, "Long-lived sharing is free during preview.")
	}
