...
type recordingWebsocketIngress struct {
	mu         sync.Mutex
	ids        []string
	affinities []executionaffinity.Key
}
...
func (i *recordingWebsocketIngress) HandleRequest(ctx context.Context, in exchange.RequestInput) (exchange.RequestOutput, error) {
	...
	i.affinities = append(i.affinities, in.ExecutionAffinity)
	i.mu.Unlock()
...
func TestResponsesWebsocket_AllocatesDistinctExchangeIdentityPerCreate(t *testing.T) {
	...
	wantAffinity, err := executionaffinity.Derive("client/x-opencode-session/v1", "alpha", "secret-marker-123")
	if err != nil {
		t.Fatal(err)
	}
	if len(ingress.affinities) != 2 || ingress.affinities[0] != wantAffinity || ingress.affinities[1] != wantAffinity {
		t.Fatal("websocket connection did not reuse one derived execution affinity")
	}
}
